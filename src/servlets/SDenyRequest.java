...
/**
 * Servlet implementation class SDenyRequest
 */
@WebServlet("/SDenyRequest")
public class SDenyRequest extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (WebSessionManager.getInstance().denyPassengerRequest(requestname, tripname, useremail, requester)==0){
			/*
			Map<String, String> deniedRequests = 
					(Map)request.getSession().getAttribute("deniedRequests");
			deniedRequests.put(deniedRequests.size()+1+"", requester);
			request.getSession().setAttribute("deniedRequests", deniedRequests);*/
			
			PrintWriter out = response.getWriter();
		    //out.println("<html><body>");

		    out.println("Denied Request. You may close this window.");
